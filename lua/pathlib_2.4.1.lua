...
function Path:resolve()
    return Path(getFullPath(self.path))
end
...
function Path:is_dir()
    local p = trim_trailing_sep(self.path)
    local pipe = io.popen('if exist "' .. p .. '\\*" (echo 1) else (echo 0)')
...
function Path.write_file(text, dst)
...
function Path:parent()
    local p = self.path:match("^(.*)\\[^\\]+$")
    return Path(p or self.path)
end
...
local function convert_lua_to_flat_string_multiline(file_path)
    ...
    for line in content:gmatch("[^\n]+") do
        ...
        if line ~= "" then
            -- convert comments to block
            local code, comment = split_comment_outside_strings(line)

            -- drop lines that are only comments or whitespace
            if code == "" and not comment then
                goto continue
            end
            ...
            ::continue::
            -- update paren_level safely, handle multi-line strings
            paren_level, in_string = count_parens_outside_strings(line, paren_level, in_string)

            -- determine whether to skip semicolon
            local skip_semicolon = false

            -- skip if inside multi-line parentheses or multi-line string
            if paren_level > 0 or in_string then
                skip_semicolon = true
            end

            -- skip if line ends with {, (, [ or keywords
            local last_char = line:match(".*(.)%s*$") or ""
            ...
            if (
                ...
                line:match("^then$") or
                line:match("^do$") or 
                line:match("^repeat$")) then

                skip_semicolon = true
            end

            -- add semicolon if safe
    ...
    -- join lines
    local str1 = table.concat(lines, " ")

    local libname = filename:gsub("%.", "_")
...
function Path.exportLua(srcFilePath, dstFilePath)
    -- normalize source
    if type(srcFilePath) == "string" then
        srcFilePath = Path(srcFilePath)
    end

    assert(getmetatable(srcFilePath) == Path, "srcFilePath must be a Path or string")

    -- check source exists and is a file
    assert(srcFilePath:exists(), "Source file does not exist: " .. tostring(srcFilePath))
    assert(srcFilePath:is_file(), "Source is not a file: " .. tostring(srcFilePath))

    -- default destination: <src>.exported
    if dstFilePath == nil then
        dstFilePath = Path(tostring(srcFilePath) .. ".exported")
    elseif type(dstFilePath) == "string" then
        dstFilePath = Path(dstFilePath)
    end

    assert(getmetatable(dstFilePath) == Path, "dstFilePath must be a Path or string")

    -- ensure destination parent exists
    local parent = dstFilePath:parent()
    if parent and not parent:is_dir() then
        parent:mkdir(true)
    end

    -- convert lua → flattened multiline string
    local texttosave = assert(
        convert_lua_to_flat_string_multiline(tostring(srcFilePath)),
        "Lua export conversion failed"
    )

    -- write output
    Path.write_file(texttosave, dstFilePath)
